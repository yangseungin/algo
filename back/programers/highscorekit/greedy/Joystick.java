			...
			while (!tmp.equals("name")) {
				...
				if (tmp.charAt(idx) != name.charAt(idx)) {
					...
					for (int i = 1; i < name.length(); i++) {
						if (tmp.charAt(idx + i>tmp.length()-1?(tmp.length()-(i+idx)):idx + i) == 'A'){
							rIdx += 1;
							break;
						}						
						...
						//1씩더하는게아니고 i씩더해야할거같은데 그리고 더하고낫 for문나가기전 lidx ridx가 0 이아니면 brea를 추가해야할
				 ...
				 * (name.charAt(i) - 'A' <= 13) rcnt += name.charAt(i) - 'A';
				 * else rcnt += 91 - name.charAt(i); rarr[i] = name.charAt(i); }
				 * 
				 * if (!String.copyValueOf(larr).equals(name) && j > 0) {
				 * lcnt++; if (name.charAt(j) - 'A' <= 13) lcnt +=
				 * name.charAt(j) - 'A'; else lcnt += 91 - name.charAt(j);
